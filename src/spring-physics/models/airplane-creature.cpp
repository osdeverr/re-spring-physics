...
AirplaneCreature::AirplaneCreature(World* world) : m_world(world) {
    ...
    for (int i = 0; i < wheel_nodes; i++) {
        ...
        config.m_springs.emplace_back(SpringConfig{30, 3, 1, node_index});
        config.m_springs.emplace_back(SpringConfig{30, 3, 5, node_index});

        config.m_springs.emplace_back(SpringConfig{30, 3, 1, node_index + 1});
        config.m_springs.emplace_back(SpringConfig{30, 3, 5, node_index + 1});

        if (i > 0) {
            config.m_springs.emplace_back(SpringConfig{30, 3, node_index, node_index - 2});
            config.m_springs.emplace_back(SpringConfig{30, 3, node_index + 1, node_index - 1});
        } else {
            config.m_springs.emplace_back(SpringConfig{30, 3, node_index, node_index + 2 * (wheel_nodes - 1)});
            config.m_springs.emplace_back(SpringConfig{30, 3, node_index + 1, node_index + 1 + 2 * (wheel_nodes - 1)});
        }
    }
    ...
    m_creature = std::make_unique<Creature>(m_world, config);
